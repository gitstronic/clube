...
begin
  with DM.aqySaldos do
    begin
      try
        SQL.Clear;
        SQL.LoadFromFile(ExtractFilePath(Application.ExeName) + 'aqySaldos.sql');
      except
        SQL.Clear;
        SQL.Add(';with a as(select conta_corrente_id,sum(pagto_valor) as Saldo from contas_receber_pagtos where pagto_data>= (select contas_correntes.data_abertura from contas_correntes where id=conta_corrente_id) group by conta_corrente_id),');
